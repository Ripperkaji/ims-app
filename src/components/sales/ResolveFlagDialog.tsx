...
import { Card, CardContent, CardHeader, CardTitle as DialogCardTitle, CardDescription as DialogCardDescription } from '@/components/ui/card'; // Aliased to avoid conflict
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
import type { Sale, SaleItem, Product, LogEntry } from '@/types';
import { ShieldCheck, PlusCircle, Trash2, Phone, Info, Landmark } from 'lucide-react';
import { useToast } from '@/hooks/use-toast';
import { mockProducts as allGlobalProductsFromData } from '@/lib/data'; // Import directly for consistency
...
interface ResolveFlagDialogProps {
  ...
  onFlagResolved: (
    ...
    updatedSaleData: Partial<Sale> & { // Ensure this structure matches what SalesPage expects
        customerName: string;
  ...
  allGlobalProducts: Product[]; // Pass global products for consistency
}
...
export default function ResolveFlagDialog({ sale, isOpen, onClose, onFlagResolved, allGlobalProducts }: ResolveFlagDialogProps) {
  ...
  const [editedItems, setEditedItems] = useState<SaleItem[]>(() => JSON.parse(JSON.stringify(sale.items))); // Deep copy
  ...
  // Recalculate dialogTotalAmount based on editedItems
  const dialogTotalAmount = useMemo(() => {
    return editedItems.reduce((sum, item) => sum + item.totalPrice, 0);
  }, [editedItems]);

  // Effect to re-initialize state when the sale prop changes (e.g., opening dialog for a different sale)
  useEffect(() => {
    if (isOpen) {
      setEditedCustomerName(sale.customerName);
      setEditedCustomerContact(sale.customerContact || '');
      setEditedItems(JSON.parse(JSON.stringify(sale.items))); // Ensure deep copy
  ...
  // Effect for hybrid payment logic
 useEffect(() => {
    if (editedFormPaymentMethod === 'Hybrid') {
      setIsHybridPayment(true);
    } else {
      setIsHybridPayment(false);
      // No need to clear hybrid fields here as they might be part of initial sale data
      // setHybridCashPaid('');
  ...
  const handleItemChange = (index: number, field: keyof SaleItem, value: string | number) => {
    ...
    if (field === 'productId') {
      ...
      // Allow current quantity if editing an existing item, otherwise check stock
      ...
      if (quantity > (stockToCheck + quantityAlreadyInSale)) { // Check against available stock + what was already in this sale
  ...
  const handleConfirmResolution = () => {
    if (!resolutionComment.trim()) {
      toast({ title: "Resolution Comment Required", description: "Please provide a comment.", variant: "destructive"});
      return;
    }
    ...
    const updatedSalePortion = {
        ...
        // status will be determined by SalesPage based on amountDue
    };
  ...
  const handleDialogClose = () => {
    // Reset state to original sale data when dialog is closed without confirming
    setEditedCustomerName(sale.customerName);
    setEditedCustomerContact(sale.customerContact || '');
  ...
  const availableProductsForDropdown = (currentItemId?: string) => 
    allGlobalProducts.filter(p => 
      (p.stock + (editedItems.find(si => si.productId === p.id)?.quantity || 0) > 0) || // Product has stock OR is already in the list
      (currentItemId && p.id === currentItemId) // Or it's the currently selected product for this item
      ...
      <DialogContent className="sm:max-w-3xl max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            <ShieldCheck className="h-5 w-5 text-green-600" /> Resolve & Adjust Sale
          </DialogTitle>
          <DialogDescription>
            Reviewing flagged sale <strong>{sale.id.substring(0,8)}...</strong>. Adjust details as necessary and provide a resolution comment.
          </DialogDescription>
        </DialogHeader>
                ...
                >
                    ...
                    {availableProductsForDropdown(item.productId).map((p) => (
                        <SelectItem key={p.id} value={p.id} 
                                    disabled={p.stock === 0 && p.id !== item.productId && !(sale.items.find(si => si.productId === p.id)) } // Complex disable logic
                        >
        ...
        <DialogFooter className="pt-4 border-t">
          ...
          <Button 
            ...
            disabled={!resolutionComment.trim() || (isHybridPayment && !!validationError) || editedItems.some(item => item.quantity <=0 && item.totalPrice > 0) || (editedItems.length > 0 && dialogTotalAmount <= 0) }
            className={(!resolutionComment.trim() || (isHybridPayment && !!validationError)) ? "bg-primary/50" : "bg-primary hover:bg-primary/90"}
          >
            <Landmark className="mr-2 h-5 w-5" /> Confirm Resolution & Save Changes
          </Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
